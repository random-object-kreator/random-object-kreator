...
class Generator1<T, A, B>(private val custom: (A, B) -> T) {
    ...
    operator fun getValue(a: Any, property: KProperty<*>): Generator1<T, A, B> {
        ...
        CreationLogic.GenericObjectFactory[type] = { _, property, token -> custom(
                CreationLogic.instantiateRandomClass(type1, type1.jvmErasure.java, token =token, kProperty = property) as A,
                CreationLogic.instantiateRandomClass(type2, type2.jvmErasure.java, token = token, kProperty = property) as B) as Any }
        return this
    }
}
...
class Generator0<T, A>(private val custom: (A) -> T) {
    operator fun getValue(a: Any, property: KProperty<*>): Generator0<T, A> {
        val type = property.returnType.arguments.first().type!!
        val type1 = property.returnType.arguments[1].type!!
        CreationLogic.GenericObjectFactory[type] = { _, property, token -> custom(CreationLogic.instantiateRandomClass(type1, type1.jvmErasure.java, token =token, kProperty = property) as A) as Any }
        return this
    }
}
